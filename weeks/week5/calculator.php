...
<form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post">
    ...
    <li>
        <input type="radio" name="gas_price" value="5.00" <?php if (isset($_POST['gas_price']) && $_POST['gas_price'] == '5.00') echo 'checked="checked"'; ?>>$5
    </li>
    <li>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Calculation logic according to Spock
    if (isset($_POST['name'], 
    $_POST['miles'], 
    $_POST['speed'], 
    $_POST['hours'], 
    ...
    $_POST['fuel_efficiency'])) {
        ...
        if (!empty($name && $miles && $speed && $hours && $gas_price && $fuel_efficiency)) {
            ...
            echo '<div class="box">
            <h2>Hello ' . htmlspecialchars($name) . '
            </h2><p>You will be travelling a total of <b>' . number_format($total_hours, 2) . ' hours</b>, <br>taking <b>' . number_format($total_days, 2) . ' days</b>.</p>
            <p>And you will be using <b>' . number_format($gallons_needed, 2) . ' gallons</b> of gas, <br>costing you <b>$' . number_format($total_cost, 2) . ' dollars</b>.</p>
            </div>';
} //end if check, and post result box
 } // end calculations
}
?>
